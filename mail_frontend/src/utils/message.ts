 ...
 */
export const showMessage = (msg: string, type: 'success' | 'error' | 'info' | 'warning' = 'success', duration?: number) => {
  // 清除之前的消息
  if (messageContainer) {
  ...
  const colorClass = type === 'success' ? 'border-green-200 bg-green-50' : 
  ...
  const iconBg = type === 'success' ? 'bg-green-500' : 
